...
class TestAccountService:
    ...
    @pytest.mark.asyncio
    async def test_get_all_accounts_success(
        self, account_service, mock_db, sample_account_data
    ):
        # Arrange
        mock_account_records = [sample_account_data]

        with patch.object(
            account_service.repository, "get_all", return_value=mock_account_records
        ) as mock_get_all:
            # Act
    ...
    async def test_get_all_accounts_empty(self, account_service):
        # Arrange
        with patch.object(
            account_service.repository, "get_all", return_value=[]
        ) as mock_get_all:
            # Act
    ...
    async def test_get_account_by_id_success(
        self, account_service, sample_account_data
    ):
        # Arrange
        with patch.object(
            account_service.repository, "get_by_id", return_value=sample_account_data
        ) as mock_get:
            # Act
    ...
    async def test_get_account_by_id_not_found(self, account_service):
        ...
        with patch.object(
            ...
            "get_by_id",
            side_effect=AccountNotFoundException(1),
        ):
            # Act & Assert
    ...
    @pytest.mark.asyncio
    async def test_get_account_by_user_id_success(
        self, account_service, sample_account_data
    ):
        # Arrange
        with patch.object(
    ...
    async def test_get_account_by_user_id_not_found(self, account_service):
        ...
        with patch.object(
            account_service.repository,
            "get_by_user_id",
            side_effect=AccountNotFoundException(999),
        ):
            # Act & Assert
            with pytest.raises(AccountNotFoundException):
    ...
    async def test_create_account_success(
        ...
        with patch.object(
            account_service.repository, "create", return_value=sample_account_data
        ) as mock_create:
            # Act
    ...
    async def test_create_account_duplicate(self, account_service):
        ...
        with patch.object(
            account_service.repository,
            "create",
            side_effect=DuplicateAccountException(123),
        ):
            # Act & Assert
    ...
    async def test_update_account_success(
        ...
        with patch.object(
            account_service.repository, "update", return_value=updated_data
        ) as mock_update:
            # Act
    ...
    async def test_update_account_no_changes(
        ...
        with patch.object(
            account_service.repository, "update", return_value=sample_account_data
        ) as mock_update:
            # Act
            result = await account_service.update_account(1, account_data)
    ...
    async def test_delete_account_success(self, account_service, mock_db):
        # Arrange
        with patch.object(
            account_service.repository, "delete", return_value=True
        ) as mock_delete:
            # Act
    ...
    async def test_deposit_success(self, account_service, mock_db, sample_account_data):
        ...
        with patch.object(
            account_service.repository, "get_balance", return_value=Decimal("1000.00")
        ) as mock_get_balance, patch.object(
            account_service.repository, "update_balance", return_value=updated_data
    ...
    @pytest.mark.asyncio
    async def test_deposit_success_with_zero_initial_balance(
        ...
        with patch.object(
            account_service.repository, "get_balance", return_value=Decimal("0.00")
        ) as mock_get_balance, patch.object(
            account_service.repository, "update_balance", return_value=account_data
        ) as mock_update_balance:
            # Act
            result = await account_service.deposit(1, Decimal("500.00"))
    ...
    async def test_transfer_success(
        ...
        with patch.object(
            account_service.repository, "get_by_id"
        ) as mock_get_by_id, patch.object(
            account_service.repository, "get_balance"
        ) as mock_get_balance, patch.object(
            account_service.repository, "update_balance"
        ) as mock_update_balance:
    ...
    @pytest.mark.asyncio
    async def test_transfer_same_account(
        self, account_service, mock_db, sample_account_data
    ):
        # Arrange
        from_account_data = sample_account_data.copy()
        ...
        with patch.object(
            account_service.repository, "get_by_id", return_value=from_account_data
        ) as mock_get_by_id, patch.object(
            account_service.repository, "get_balance", return_value=Decimal("1000.00")
        ) as mock_get_balance, patch.object(
            account_service.repository, "update_balance", return_value=from_account_data
        ) as mock_update_balance:
            # Act
    ...
    async def test_create_and_retrieve_account(self, account_service, mock_db):
        ...
        with patch.object(
            account_service.repository, "create", return_value=created_account_data
        ) as mock_create, patch.object(
            account_service.repository, "get_by_id", return_value=created_account_data
        ) as mock_get_by_id:
            # Act - Create
    ...
    @pytest.mark.asyncio
    async def test_get_all_accounts_with_pagination(
        self, account_service, sample_account_data
    ):
        # Arrange
